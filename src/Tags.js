...
const Tags = ({ blogsData }) => {
  ...
  useEffect(() => {
    ...
    let blogsCount = 0;
    blogsData.map((blog) =>
      blog.tags.map((tag) => (tag === clickedTag ? blogsCount++ : null))
    );
  ...
  return (
    <div className="tags-page">
      <div className="blogs-tagged">
        <h1>
          You're Currently on <span>{clickedTag}</span> Page
        </h1>
        {blogsData.map((blog) =>
          blog.tags.map((tag) =>
            tag === clickedTag ? (
        ...
        {blogsData.map((blog) =>
          blog.tags.map((tag) => {
            if (!tagsAdded.includes(tag) && tag !== clickedTag) {
              tagsAdded.push(tag);
              return (
                <Link to={`/tag/${tag}`} onClick={() => setRerender(!rerender)}>
                  <div className="tag-container" key={tag}>
                    <div className="tag-name">{tag}</div>
                    <div className="tag-info">
                      {tagsCount[tag]}{" "}
                      {`${tagsCount[tag] === 1 ? "Article" : "Articles"}`}
                    </div>
                  </div>
                </Link>
              );
