...
const Blogs = ({ blogsData }) => {
  ...
  for (let index = 0; gridAreas.length < blogsData.length; index++) {
    gridAreas.push(
      `${Number(gridAreas[index].split(" / ")[0].split(" ")) + 4} / ${Number(
        gridAreas[index].split(" / ")[1].split(" ")
      )} / ${
        Number(gridAreas[index].split(" / ")[2].split(" ")) + 4
      } / ${Number(gridAreas[index].split(" / ")[3].split(" "))}`
    );
  }
  ...
  return (
    <div className="blogs-area">
      <div className="blogs">
        {blogsData.map((blog) => (
          <div
            className="blog hidden"
            key={blog.id}
            style={{ gridArea: `${blog.gridArea}` }}
          >
